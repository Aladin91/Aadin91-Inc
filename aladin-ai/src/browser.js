...
export class ModelPackage {
  static create(model,{name='',featureNames=[],targetName='',trainingRows=null,notes='',tags=[]}={}){
    return {
      ...
      version:1,
      metadata:{name,modelType:model.constructor.name,engineVersion:'0.2.0',featureNames,targetName,trainingRows,notes,tags,createdAt:new Date().toISOString(),schemaFingerprint:schemaFingerprint({featureNames,targetName})},
...
export class IndexedDBModelStore {
  ...
  async loadModel(name,registry){const pkg=await this.get(name);return pkg?ModelPackage.restore(pkg,registry):null;}
}

export async function processInBatches(items,processor,{batchSize=100,onProgress=null,signal=null}={}){
  const out=[];for(let start=0;start<items.length;start+=batchSize){if(signal?.aborted)throw new DOMException('Operation aborted','AbortError');const batch=items.slice(start,start+batchSize);for(let i=0;i<batch.length;i++)out.push(await processor(batch[i],start+i));onProgress?.({processed:Math.min(start+batch.length,items.length),total:items.length,progress:Math.min(start+batch.length,items.length)/Math.max(items.length,1)});await new Promise(resolve=>setTimeout(resolve,0));}return out;
}
